...
pipeline {
			...
			stages {
				stage('Source'){
					steps{
						checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'ABDSonata', url: 'https://github.com/ABDSonata/Online-Examination-System/']]])
					}
				}
				stage('Build') {
    					steps {
